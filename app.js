...
function concluirTarefa(tarefa) {
    var _a;
    tarefa.concluir();
    var listaItems = listaTarefas.getElementsByTagName('li');
    for (var i = 0; i < listaItems.length; i++) {
        var item = listaItems[i];
        if ((_a = item.textContent) === null || _a === void 0 ? void 0 : _a.includes(tarefa.descricao)) {
            item.textContent = "".concat(tarefa.descricao, " - Prazo: ").concat(tarefa.prazo.toLocaleDateString(), " - Prioridade: ").concat(tarefa.prioridade, " - Status: ").concat(tarefa.status);
        }
    }
}
// Adiciona um botão para concluir a tarefa, caso deseje
var concluirBotao = document.createElement('button');
concluirBotao.textContent = 'Concluir Tarefa';
document.body.appendChild(concluirBotao);
concluirBotao.addEventListener('click', function () {
    var tarefa = tarefas[0]; // Exemplo de tarefa para ser concluída
